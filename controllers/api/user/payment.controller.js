    ...
    async create(req, res) {
        try {
            ...
            if (!userId || !productIds?.length)
                return error(res, 400, 'Thiếu thông tin người dùng hoặc sản phẩm.');
            ...
            const selectedItems = cart.products.filter(p => productIds.includes(p.productId));
            ...
            const stockData = await StockEntry.aggregate([
                { $match: { productId: { $in: productIdsList }, status: 'imported', remainingQuantity: { $gt: 0 } } },
                { $group: { _id: '$productId', totalStock: { $sum: '$remainingQuantity' } } }
            ]);

            // Map stock theo productId
            const stockMap = stockData.reduce((acc, s) => {
                acc[s._id.toString()] = s.totalStock;
                return acc;
            }, {});

            // Cập nhật isActive tạm thời dựa trên tồn kho
            const inactiveProducts = productDetails.filter(p => !p.isActive || (stockMap[p._id.toString()] || 0) <= 0);

            if (inactiveProducts.length > 0) {
                return error(
                    res,
                    400,
                    `Không thể đặt hàng. Sản phẩm "${inactiveProducts.map(p => p.name).join(', ')}" hiện không còn kinh doanh hoặc hết hàng. Vui lòng làm mới giỏ hàng trước khi tiếp tục.`
                );
            }
