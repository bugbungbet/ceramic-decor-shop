...
class OrderController {
    async overview(req, res) {
        try {
            ...
            if (q) {
                ...
                // Tìm userId tương ứng tên người nhận match
    ...
    //         // === Lấy thông tin đơn hàng + populate nếu cần
    ...
    //         // === Tổng số item trong đơn
    //         const totalItems = await OrderItem.countDocuments({ orderId: id });
    //         const totalPages = Math.ceil(totalItems / limit);

    //         // === Lấy danh sách item trong đơn (phân trang)
    ...
    //         const startIndex = (page - 1) * limit;
    //         const endIndex = Math.min(startIndex + orderItems.length, totalItems);
    ...
    async detail(req, res) {
        try {
            ...
            // Lấy tất cả OrderItem trong đơn
            const orderItems = await OrderItem.find({ orderId: id })
